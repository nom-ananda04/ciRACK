...
Reads the 34950A hardware counter/totalizer (slot 8, counter channel 301)
via raw PyVISA. Logs the accumulated rising-edge count (no streaming).

Commands verified against the 34980A Programmer's Reference
(SENSe Subsystem > Digital > COUNter commands):

  COUN:FUNC TOT,(@8301)      select totalize function  <- [SENSe:]COUNter:FUNCtion
  COUN:SLOP POS,(@8301)      count rising edges        <- [SENSe:]COUNter:SLOPe
  COUN:THR:VOLT 1.5,(@8301)  input threshold voltage   <- [SENSe:]COUNter:THReshold:VOLTage
  COUN:TOT:CLE:IMM (@8301)   zero the count            <- [SENSe:]COUNter:TOTalize:CLEar:IMMediate
  COUN:TOT:DATA? (@8301)     read the count            <- [SENSe:]COUNter:TOTalize[:DATA]?

Counter channels on the 34950A are s301 (counter 1) and s302 (counter 2)
per the Programmer's Reference ("Select from s301 or s302").

Wiring: count signal on the COUNTER CH301 count/CLK terminal, with the
source's ground bridged to the counter common. In totalizer mode the
counter free-runs; the external GATE terminal is not used unless a gate
